...
class _AddExpenseScreenState extends State<AddExpenseScreen> {
  ...
  final _categories = <_Cat>[
    ...
    _Cat(
      ...
      const Color(0xFFE06157),
    ),
    _Cat(
      'Shopping',
      Icons.shopping_bag_rounded,
      const Color(0xFFFFF9EC),
      const Color(0xFFE7B039),
    ),
    _Cat(
      'News Paper',
      Icons.menu_book_rounded,
      const Color(0xFFFFF7ED),
      const Color(0xFFDFA14A),
    ),
    _Cat(
      'Transport',
      Icons.directions_bus_rounded,
      const Color(0xFFF2EFFF),
      const Color(0xFF7D56F3),
    ),
    _Cat(
      'Rent',
      Icons.home_rounded,
      const Color(0xFFFFF7EF),
      const Color(0xFFE7A568),
    ),
  ];
  ...
  Widget _filledField({required Widget child, Widget? trailing}) {
    return Container(
      ...
      height: 52,
      child: Row(
        children: [Expanded(child: child), if (trailing != null) trailing],
      ),
    );
  }
              ...
              child: Column(
                ...
                children: [
                  Container(
                    ...
                    child: Icon(
                      c.icon,
                      color: isSel ? const Color(0xFF3A6FF7) : c.fg,
                    ),
                  ),
  ...
  void _onSave() {
    ...
    print(
      '[AddExpense] Dispatch SaveExpense: category="${_categoryCtrl.text}", amount=$parsed, currency=$_selectedCurrency',
    );
