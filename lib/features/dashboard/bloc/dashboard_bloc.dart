...
class DashboardBloc extends Bloc<DashboardEvent, DashboardState> {
  ...
  List<ExpenseModel> filterBy(
    ...
    switch (f) {
      case DashboardFilter.thisMonth:
        ...
        end   = DateTime(localNow.year, localNow.month + 1, 1); // next month 00:00
        break;

      case DashboardFilter.last7Days:
      // last 7 calendar days INCLUDING today
        final today0 = DateTime(localNow.year, localNow.month, localNow.day);
        start = today0.subtract(const Duration(days: 6)); // 6 back + today = 7 days
  ...
  void _emitPage(List<ExpenseModel> filtered, int page, DashboardFilter filter, Emitter<DashboardState> emit) {
    filtered.sort((a,b)=>b.date.compareTo(a.date));
    final start = (page - 1) * _pageSize;
    final end = (start + _pageSize).clamp(0, filtered.length);
    final pageItems = start < filtered.length ? filtered.sublist(start, end) : <ExpenseModel>[];
    final hasMore = end < filtered.length;


    final income = filtered.where((e)=> e.usdAmount > 0).fold<double>(0.0, (a,b)=> a + b.usdAmount);
    final expense = filtered.where((e)=> e.usdAmount < 0).fold<double>(0.0, (a,b)=> a + b.usdAmount.abs());
  ...
  Future<void> _onChangeFilter(ChangeFilter e, Emitter<DashboardState> emit) async {
